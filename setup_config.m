...
%Distributed accourding to the MIT licens
...
%@ebb: min_trajec_length added
%@ebb: modality 5 cross val added
%%19-Jun-2015 #173,180@ebb:new input argument for not dispalying the flow
%                     field
%10 oct 2017 adding total shuffle (not preserving anything) when is_shuffled_within_events>1
...
%
%        
%       min_trajec_length (1x1). Do not classifiy as "convergent" or not trajectories shorter than this
%                         number of bins
%
...
%(#Line@author: Change description). This will be useful if assistance is
%required
%19-Jun-2015 #173,180@ebb:new input argument for not dispalying the flow field
%10 Oct 2017 If "shuffled within events>1, do a totla shuffle not
%respecting labels.
%__________________________________________________________________________
...
if (nargin<3)||(is_shuffled_events<=0)
    ...
    disp('   Shuffle task-epochs');%Suffles blocks as specified by the first column in labels
end
if (nargin<4)||(is_shuffled_within_events<=0)
    ...
    if is_shuffled_within_events>1
         disp('   Total shuffle')
    end 
end
...
if (nargin<8)||(is_multiple_discriminant<=0)
    ...
    disp('   No multiple discriminant discriminant-based statistics will be reported');
else
...
elseif (xval_type<0)
    ...
    warning('setup_config:no_cross','No cross validation performed: be sure that regularization is correrct'),
    xval_type=-1;
elseif (xval_type==4)
    disp('      Only first trial is used for estimation, remaining are for testing '),
    xval_type=4;
elseif (xval_type==5)
    disp('      Causal, leave-one-out single-trial validation '),
    xval_type=5;
else
...
if (nargin<13)
    min_trajec_length=3;
    disp(['   Task-epoch specific trajectories are longer than ',num2str(min_trajec_length),...
        ' time bins']),
elseif (min_trajec_length<=0)
    min_trajec_length=3;
    disp(['   Task-epoch specific trajectories are longer than ',num2str(min_trajec_length),...
        ' time bins']),
else
...
if (nargin<17)||(balance_task_epochs<=0)
...
if (nargin<22)||(no_flow_field<=0)
    no_flow_field=0;%It shows the flow field by default
else
    %     disp(['Warning: Exponential kernel used instead of polynomial. ',...
    %         ' Thus "order" is irrelevant), witdth ',num2str(is_exponential_kernel)]);
    disp('Flow field ommited in 3D plots')
end
...
conf.min_trajec_length=min_trajec_length;
conf.do_dcm=do_dcm;
...
conf.no_flow_field=no_flow_field;
end
