...
%V4.1b
%To compare JS of 1 class against the others see this file in deprecated
%folder. This file is generic.
...
%Inputs:
%
%   data (n_time x subspace dimensionality) = Neural responses over time, spanned in an
...
for i=1:n_labels
    ...
    if chance_priors>0
        priors(i)=1/n_labels;
        %priors(i)=.5;%This is not correct
    else
    ...
    if balance_trajec_length>0, max_trajec_length=balance_trajec_length;
    else 
...
%There is a bug in versions before 28 jun 2015, class_j_s(n_labels) was
%never computed
class_j_s(n_labels)=sum(j_s(n_labels,:));
%class_j_s=sum(j_s)./(n_labels-1);
%I beleive that was wrong. This makes more sense to me aug 18
%class_j_s=class_j_s./counter;%just to check
%class_j_s(n_labels)=mean(j_s(n_labels,:));%AUg 18 look at previous
%versions to see the error
