...
%E {Y(t, w)} = log{ft(w)}, where ft is the fourier transform at each trial
...
load('A201To7NoTestErrors')
%ntrials=data(end,5);
ntrials=max(data(:,end));
%trial_size=2500;
trial_size=4600;
freqs=zeros(trial_size,ntrials);
limit_trial=6;%Last trial to be tested
axes=1;%x_axes=1, y_axes=2 etc.
for i=1:ntrials-1 %tyically 11 trials, 300 init cond each
    
  lower_limit=((i-1)*trial_size)+1;
  upper_limit=i*trial_size;
  if upper_limit>length(data(:,1))
      upper_limit=length(data(:,1));
  end
...
ntrials2=7-1;
subplot(211)
errorbar((1:ntrials2),mean(freqs(:,1:ntrials2)),(std(freqs(:,1:ntrials2))./sqrt(trial_size)))
subplot(212)
bar(mean(freqs(:,1:ntrials2)))
