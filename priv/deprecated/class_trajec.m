function [basic_stats,add_stats]=class_trajec(data,labels,time,chance_priors,balance_trajec_length,min_trajec_length,full_stats_disp)
%V4.1 beta, not guaranteed.
%WARNIGN 30 AUG 18 THIS IS JUST FOR COMPARING 1 AGAINST OTHERS
...
%#484 and below@ebb:JS error
%#562, 571@ebb% There is a bug in versions before 28 jun 2015, class_j_s(n_labels) was
%          never computed.Counter per class was wrong. This has been never
%          used before and hence it was overlooked. Fixed.
...
confidence_class=95;%96.5;%99.6;%97 for ozone;%99.5;%In percent: threshold for detecting if a concrete pattern is an outliner,
...
%May 2018. This was all wrong, the priors were set to 0.5 by default before
priors=(1/n_labels).*ones(1,n_labels);means=cell(1,n_labels);covariances=cell(1,n_labels);
...
for i=1:n_labels,
    ...
    if chance_priors>0,
        priors(i)=1/n_labels;%May 2018. This was all wrong, the chance priors were set to 0.5 by default before
        %priors(i)=.5;%This is not correct,I ma only checking aug 2018
    else
    ...
    if balance_trajec_length>0, max_trajec_length=balance_trajec_length; else max_trajec_length=Inf; end
...
for i=1:n_patt,
    ...
    if (i==1)||((c==labels(i-1))&&(trajec_pointer<=max_trajec_length))
        ...
        %cannot really be considered as divergent
        %
        %
        if (i>1)&&...
                (( misscla_consec>0)&&(trajec_pointer>=min_trajec_length))%||( misscla_consec==trajec_pointer) %... unless all those vectors where incorrectly assigned (deactivated)
            diverg_trajec=diverg_trajec+1;
            cum_diverg_trajec_class(set_labs==c)=cum_diverg_trajec_class(set_labs==c)+1;
        end
        %
        %Too short class-specific trajectories perhpas should not be
        %reported anymore.
    ...
    for j=1:n_labels
        ...
        %Aug 29 2017 set to 100 due to issues
        regul_attempts=20;
...
%disp([num2str(diverg_trajec),'/',num2str(n_trajects),' divergent trajects.']),
...
for i=1:n_labels
    ...
    for j=1:n_labels
        ...
        %Agug 18 what an embarraign and surpising mistake: pdfs were not
        %normalized. Oh dear.Relax thou,it was normalizes anyway inside the
        %KL but may have affected M
        pdf_i=cum_posterior_probs(i,:)'./sum(cum_posterior_probs(i,:)');
        pdf_j=cum_posterior_probs(j,:)'./sum(cum_posterior_probs(j,:)');
        ...
        %if i<j %Retain only the different ones for the final JS.
        %k=k+1;
        if (    isnan( j_s(i,j))||(abs(  j_s(i,j))>1)|| ( j_s(i,j)<0)) %aug 18
            if full_stats_disp>0
                warning('class_trajec:NaN_divergence_error', [' Jensen-Shanon is not a valid number please trace the comparison ( class i, class j)=',num2str(i),',',num2str(j)]);
            ...
            if i<j
                ...
                JSsErr(k)=j_s_error(i,j);
            end
            %Sum per class, for all of them
            if (j~=i)
                if (i==1),     %Novelty 30 aug 18 see below
                    class_j_s(i)=class_j_s(i)+j_s(i,j);%Bug in previous version
                    %                                   never used before:
                    counter(i)=counter(i)+1;%there is no need. aug 18: Really?
                elseif (j>1),
                    class_j_s(i)=class_j_s(i)+j_s(i,j);%Bug in previous version
                    %                                   never used before:
                    counter(i)=counter(i)+1;%there is no need. aug 18: Really?
    ...
    %counter(i)=sum(j_s(i,:)>0);%I think is OK aug 2018
...
%Aug 2018 "New" idea of the js per class (class JS): mean of 1 against all (for isnatce, 3 comparisons for 4 classes),
%and then the mean of the rest of them against each other, allwing
%repetition.
%II.2.4 Global tests, both uni- and multivariate
%JS storing
JSs=JSs(JSs>-1);JSsErr=JSsErr(JSsErr>-1);
basic_stats.js=mean(JSs);
basic_stats.jsErr=mean(JSsErr);
