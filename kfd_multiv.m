function [r_data_ref,basic_stats,add_stats, issue_matrix]=kfd_multiv(data_ref,labels_ref,order,reg,...
    data_val,labels_val,balance_trajec_length,min_trajec_length,full_stats_disp)
...
if nargin<9, full_stats_disp=0; end
if nargin<8, min_trajec_length=10; end %See meaning of this parameter in "class_trajec.m"
if nargin<7, balance_trajec_length=30; chance_priors=0; else chance_priors=1; end
...
if (n_labels_val>1)&&(n_labels_ref>1)
    ...
    issue_matrix=0; %checks for non convergent eigenvalues or infinite ones may 18
    for i=1:n_patt_ref
        for j=i:n_patt_ref
    ...
    %Regularizing
    mK=abs(mean(mean(K)));
    %mK=1;%This was used from may 2018 to aug 2018 with the new data I guess
    ...
    %nans_evals=isnan(evals);
    %if sum(nans_evals), disp('*TROUBLE in kfd_multiv*: there were Nan evals, set to zero and block maintained'), end
    %evals(nans_evals)=0;%That will make zero the Nans and will be sorted at the bottom

    %Warning: eigenvalues have to be ordered.
    %May 2018. However, before that I will transform NaN eigenvalues to 
    %zero, otherwise they are ordered on the top of the list, then r_data_ref is 
    %zero and everything is messed up
    %
    %inf_evals=isinf(evals);
    %evals(inf_evals)=0;%That will make zero the inf and will thus never be selected

    %if sum(inf_evals), disp('issues with convergence-block skipped'); issue_matrix=1; end%May 18. Just mistrust these outputs
    %evals(nans_evals)=0;%That will make zero the Nans and will be sorted at the bottom
    %if (sum(inf_evals)), disp('issues with convergence-block skipped'); issue_matrix=1; end%May 18. Just mistrust these outputs
...
%     end
  ...
  % if ~any(any(r_data_ref)) 
       %disp('issues with convergence-block skipped'), 
   %    issue_matrix=1; 
   %end%May 18. Just mistrust these outputs
    ...
    if ~any(any(r_data_val))
        disp('issues with convergence- consider block skip'),
        issue_matrix=1; 
    end%May 18. Just mistrust these outputs. 
    %Flag dIsconnected temporarily in august 18 end in kfd_corss_val
    ...
    %chance_priors=0;
    %[basic_stats,add_stats]=class_trajec_uncertainty(r_data_val,class_lab_val,t_val,chance_priors,balance_trajec_length,min_trajec_length,full_stats_disp);
    [basic_stats,add_stats]=class_trajec(r_data_val,class_lab_val,t_val,chance_priors,balance_trajec_length,min_trajec_length,full_stats_disp);
    %
    %If some label was not present in validation set, chance predictions
    %will be added to the real ones
    
    %Optional: If the refernece set lacks of a label which is in the validation one, do the same
    
    if n_labels_val>n_labels_ref
        reference_labels=set_labs_val;
    else
        reference_labels=set_labs_ref;
    end
    
    basic_stats=fill_predic(basic_stats,reference_labels,set_labs);
else
